...
#
# NOTE: If this doesn't work properly, first change directory to "jni" and use
# the Makefile there:
#	cd jni
#	make [arguments]
...
# "make first", so that the target from "jni/auto/config.mk" is picked
# up properly when config didn't run yet.  Doing "make all" before configure
# has run can result in compiling with $(CC) empty.

first:
	@if test ! -f jni/auto/config.mk; then \
		cp jni/config.mk.dist jni/auto/config.mk; \
	fi
	@echo "Starting make in the jni directory."
	@echo "If there are problems, cd to the jni directory and run make there"
	cd jni && $(MAKE) $@
...
#
# - Update Vim version number.  For a test version in: jni/version.h, Contents,
#   MAJOR/MINOR above, VIMMAJOR and VIMMINOR in jni/Makefile, README*.txt,
#   runtime/doc/*.txt and nsis/gvim.nsi. Other things in README_os2.txt.  For a
#   minor/major version: jni/GvimExt/GvimExt.reg, jni/vim.def, jni/vim16.def,
#   jni/gvim.exe.mnf.
...
#   warnings).
# - If you have valgrind, enable it in jni/testdir/Makefile and run "make
#   test".  Enable EXITFREE, disable GUI, scheme and tcl to avoid false alarms.
#   Check the valgrind output.
# - If you have the efence library, enable it in "jni/Makefile" and run "make
#   test".  Disable Python and Ruby to avoid trouble with threads (efence is
#   not threadsafe).
# - Adjust the date and other info in jni/version.h.
# - Correct included_patches[] in jni/version.c.
...
# - Check if jni/Makefile, jni/testdir/Makefile and jni/feature.h don't contain
...
# - Run jni/bigvim.bat ("nmake -f Make_mvc.mak GUI=yes OLE=yes IME=yes ...)
...
#	gvimext.dll in jni/GvimExt
#	gvimext64.dll in jni/GvimExt
#	VisVim.dll in jni/VisVim
...
# - "nmake -f Make_mvc.mak GUI=yes"
# Or run jni/bigvim64.bat for an OLE version.
#
...
# OK to put it in the archive.
	cp -f jni/config.mk.dist dist/$(VIMRTDIR)/jni/auto/config.mk
# Create an empty config.h file, make dependencies require it
	touch dist/$(VIMRTDIR)/jni/auto/config.h
# Make sure configure is newer than config.mk to force it to be generated
	touch dist/$(VIMRTDIR)/jni/configure
...
# Same for cs.cp1250.po, pl.cp1250.po and sk.cp1250.po.
	touch dist/$(VIMRTDIR)/jni/po/ja.sjis.po
	touch dist/$(VIMRTDIR)/jni/po/cs.cp1250.po
	touch dist/$(VIMRTDIR)/jni/po/pl.cp1250.po
	touch dist/$(VIMRTDIR)/jni/po/sk.cp1250.po
	touch dist/$(VIMRTDIR)/jni/po/zh_CN.cp936.po
	touch dist/$(VIMRTDIR)/jni/po/ru.cp1251.po
	touch dist/$(VIMRTDIR)/jni/po/uk.cp1251.po
# Create the archive.
...
dosrt_unix2dos: dist prepare no_title.vim
	...
	cd jni && MAKEMO=yes $(MAKE) languages
	tar cf - \
		$(RT_ALL) \
		| (cd dist/vim/$(VIMRTDIR); tar xf -)
	tar cf - \
...
# zh_CN.mo / zh_CN.cp936.mo, uk.mo / uk.cp1251.mo and ru.mo / ru.cp1251.mo.
	for i in $(LANG_DOS); do \
	      if test "$$i" != "jni/po/ja.mo" -a "$$i" != "jni/po/pl.mo" -a "$$i" != "jni/po/cs.mo" -a "$$i" != "jni/po/sk.mo" -a "$$i" != "jni/po/zh_CN.mo" -a "$$i" != "jni/po/ru.mo" -a "$$i" != "jni/po/uk.mo"; then \
		n=`echo $$i | sed -e "s+jni/po/\([-a-zA-Z0-9_]*\(.UTF-8\)*\)\(.sjis\)*\(.cp1250\)*\(.cp1251\)*\(.cp936\)*.mo+\1+"`; \
...
dosbin_ole: dist no_title.vim dist/$(COMMENT_OLE)
	...
	cp jni/VisVim/VisVim.dll dist/vim/$(VIMRTDIR)/VisVim.dll
	cp jni/VisVim/README_VisVim.txt dist/vim/$(VIMRTDIR)
